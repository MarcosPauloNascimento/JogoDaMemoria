...
main()
{
    ...
    */
    ...
    int tela=0, mapa=0, aux=0, fase = 0, idCor = 0, sequencia = 0;
    int tamanhoNome = 0;
    int corSorteada[15], somaCliques = 0, totalGeral = 0;
    int i =0,x = 0;
    int status = 0; //DESLIGADO = 0 LIGADO = 1
    fila f1, f2;
    listaRanking L;
    jogada j;
    j.record=0;
    ...
    initwindow (400,700,"Jogo da Memória - Desenvolvido por Marcos Paulo",500,0);

    readimagefile("desligado.jpg",0,0,400,700);
    ...
    do
    {
        ...
        if(status==0 && mousex() > 110 && mousex() < 143 && mousey() > 22 && mousey() < 34 && ismouseclick(WM_LBUTTONDOWN)){
            ligaCelular();
        ...
        if(mousex() > 85 && mousex() < 120 && mousey() > 610 && mousey() < 630 && ismouseclick(WM_LBUTTONDOWN))
        {
           ...
           if(tela==4 && mousex() > 35 && mousex() < 62 && mousey() > 425 && mousey() < 458 && ismouseclick(WM_LBUTTONDOWN))
            ...
            if(tela==6 && mousex() > 211 && mousex() < 340 && mousey() > 501 && mousey() < 567 && ismouseclick(WM_LBUTTONDOWN)){
                ...
                tela=8;
                readimagefile("ranking1.jpg", 0, 0, 400, 700);
                imprimirLista(L);
                strcpy(nomeJogador,"\0");
                strcpy(pontos,"\0");
                totalGeral=0;
        ...
        //DESLIGAR O CELULAR ===== IMPLEMENTAR
        /*if(status==1 && mousex() > 110 && mousex() < 143 && mousey() > 22 && mousey() < 34 && ismouseclick(WM_LBUTTONDOWN)){
            ligaCelular();
            readimagefile("principal.jpg", 0, 0, 400, 700);
            tela=0;
            status=0;
